...
/*   Updated: 2025/01/24 11:15:12 by rlebaill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	count(char *s, int count)
{
	int	i;
...
static char	*next_word(char *s, t_mini *mini, int i, int j)
{
	...
	if (!word)
		return (NULL);
	while (j < ft_len_without_quote(s, 0, mini, 0) && ++i + 1 && s[i])
	{
		if (s[i] == '$' && s[i + 1]
...
char	**ft_split_quote(char *s, t_mini *mini)
{
	...
	split = malloc(sizeof(char *) * (count(s, 0) + 1));
	i = 0;
	j = 0;
